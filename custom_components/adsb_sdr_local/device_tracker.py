...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    @callback
    def _handle_update():
        # add any newly-seen aircraft (existing ones update via coordinator)
        _add_new()
...
class ADSBAircraftTracker(CoordinatorEntity[ADSBCoordinator], TrackerEntity):
    ...
    @property
    def name(self) -> str:
        # Prefix with plane emoji so the built-in Map badge looks like a plane.
        ac = self.coordinator.get_by_hex(self._hex)
        label = None
        if ac and ac.flight:
            label = ac.flight.strip()
        if not label:
            label = self._hex.upper()
        return f"✈ {label}"
